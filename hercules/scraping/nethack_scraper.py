...
def get_monsters(soup: BeautifulSoup):
    monsters = {}
...
def parse_monster_info(monster_url: str) -> dict:
    ...
    return {k: v for k, v in monster_data.items() if k in EXPECTED_FIELDS}


if __name__ == "__main__":
    response = requests.get(MONSTERS, headers=headers)
    soup = BeautifulSoup(response.content, "html.parser")
    monster_dict = get_monsters(soup)

    print(response)

    monsters = {}
    for k in tqdm(monster_dict.keys()):
        monsters[k] = parse_monster_info(monster_dict[k])
